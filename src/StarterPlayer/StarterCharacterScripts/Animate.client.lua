...
function setRunSpeed(speed)
	local runSpeed = (speed * 1.25) / getHeightScale()
	local WalkScale = ((script.Walkspeed.Value/16)* 1.25) / getHeightScale()
	local RunScale = ((script.Runspeed.Value/16)* 1.25) / getHeightScale()
	if runSpeed ~= currentAnimSpeed then
		if runSpeed <= WalkScale+.01 then
			currentAnimTrack:AdjustWeight(1.0)		
			runAnimTrack:AdjustWeight(smallButNotZero)
		elseif runSpeed < RunScale then
			if (RunScale-runSpeed) <= smallButNotZero then
				currentAnimTrack:AdjustWeight(smallButNotZero)		
				runAnimTrack:AdjustWeight(math.floor((1-(RunScale-runSpeed))*1000)/1000)
			elseif RunScale-runSpeed>=1-smallButNotZero then
				currentAnimTrack:AdjustWeight(math.floor((RunScale-runSpeed)*1000)/1000)		
				runAnimTrack:AdjustWeight(smallButNotZero)
			else
				currentAnimTrack:AdjustWeight(math.floor((RunScale-runSpeed)*1000)/1000)	
				runAnimTrack:AdjustWeight(math.floor((1-(RunScale-runSpeed))*1000)/1000)
